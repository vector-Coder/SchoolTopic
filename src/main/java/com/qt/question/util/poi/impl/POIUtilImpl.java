package com.qt.question.util.poi.impl;
...
//                    列自适应宽度
    ...
    @Override
    public List<List<Object>> getExcelLinePartData(File excelFile, String sheetName, int startIndex, int endIndex) throws IOException, InvalidFormatException {
        if (!excelFile.exists()) {
            throw new RuntimeException("文件不存在");
        }
        if (sheetName == null || "".equals(sheetName.trim())) {
        ...
        for (int index = startIndex; index < endIndex; index++) {
           List<Object> list = this.getRowValueList(sheet.getRow(index));
            data.add(list);
        }
        return data;
    }
     ...
     * @return List
     */
    private List<Object> getRowValueList(Row row){
        //获得行的当单元格数量
        int cellLength = row.getLastCellNum();
        List<Object> list = new ArrayList<>();
        for (int cellIndex = 0; cellIndex < cellLength; cellIndex++) {
